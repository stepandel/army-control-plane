...
function drawSlackIcon(ctx, cx, cy, size) {
  ...
  const g = s * 0.25;
  const r = s * 0.22;
  drawCircle(ctx, cx - g, cy - g - s * 0.15, r);
  ctx.fillStyle = c.slackBlue;
  ctx.fill();
  drawCircle(ctx, cx + g + s * 0.15, cy - g, r);
  ctx.fillStyle = c.slackGreen;
  ctx.fill();
  drawCircle(ctx, cx - g - s * 0.15, cy + g, r);
  ctx.fillStyle = c.slackYellow;
  ctx.fill();
  drawCircle(ctx, cx + g, cy + g + s * 0.15, r);
  ctx.fillStyle = c.slack;
  ctx.fill();
  ctx.lineCap = "round";
  ctx.lineWidth = r * 1.4;
  ctx.strokeStyle = c.slackBlue;
  ctx.beginPath();
  ctx.moveTo(cx - g, cy - g - s * 0.15);
  ctx.lineTo(cx - g, cy + g);
  ctx.stroke();
  ctx.strokeStyle = c.slackGreen;
  ctx.beginPath();
  ctx.moveTo(cx + g + s * 0.15, cy - g);
  ctx.lineTo(cx - g, cy - g);
  ctx.stroke();
  ctx.strokeStyle = c.slack;
  ctx.beginPath();
  ctx.moveTo(cx + g, cy + g + s * 0.15);
  ctx.lineTo(cx + g, cy - g);
  ctx.stroke();
  ctx.strokeStyle = c.slackYellow;
  ctx.beginPath();
  ctx.moveTo(cx - g - s * 0.15, cy + g);
  ctx.lineTo(cx + g, cy + g);
  ctx.stroke();
...
function drawLinearIcon(ctx, cx, cy, size) {
  ...
  ctx.clip();
  ctx.strokeStyle = c.linear;
  ctx.lineWidth = 2.5;
  ctx.lineCap = "round";
  for (let i = -6; i <= 6; i++) {
    const offset = i * (r * 0.3);
    ctx.beginPath();
    ctx.moveTo(cx + offset - r, cy + r);
...
function drawGitHubIcon(ctx, cx, cy, size) {
  const r = size * 0.32;
  drawCircle(ctx, cx, cy, r);
  ctx.fillStyle = c.github;
  ctx.fill();
  ctx.fillStyle = c.bgCard;
  const s = r * 0.55;
  drawCircle(ctx, cx, cy - s * 0.15, s * 0.7);
  ctx.fill();
  roundRect(ctx, cx - s * 0.5, cy + s * 0.1, s, s * 0.7, s * 0.15);
  ctx.fill();
  ctx.beginPath();
...
function drawArrow(ctx, x, y, color) {
  ctx.save();
  ctx.fillStyle = color || c.textMuted;
  ctx.beginPath();
  ctx.moveTo(x, y - 5);
  ctx.lineTo(x + 8, y);
  ctx.lineTo(x, y + 5);
...
async function generateOGImage(svgBuf) {
  ...
  topLine.addColorStop(0, "rgba(108,92,231,0)");
  topLine.addColorStop(0.3, c.primary);
  topLine.addColorStop(0.7, c.accent);
  topLine.addColorStop(1, "rgba(0,210,255,0)");
  ...
  ctx.fillText("Vera", 72, 220);
  const subGrad = ctx.createLinearGradient(72, 250, 500, 320);
  subGrad.addColorStop(0, c.text);
  subGrad.addColorStop(1, c.primaryLight);
  ctx.fillStyle = subGrad;
  ctx.font = "600 34px Inter, sans-serif";
  ctx.fillText("AI engineering agent", 72, 275);
  ctx.fillText("for your team", 72, 320);
  ...
  // CTA pill
  roundRect(ctx, 72, 410, 200, 44, 22);
  ...
  cards.forEach((card) => {
    ...
    ctx.setLineDash([8, 5]);
    ctx.beginPath();
    ctx.moveTo(card.x + cardW / 2, card.y + cardH + 4);
    ctx.lineTo(veraX + cardW / 2, veraY - 4);
    ctx.stroke();
    ctx.restore();
  });
  ...
  // Draw cards
  cards.forEach((card) => {
    roundRect(ctx, card.x, card.y, cardW, cardH, 16);
    ctx.fillStyle = c.bgCard;
    ctx.fill();
    ...
    ctx.fillRect(card.x, card.y, cardW, 3);
    ctx.restore();
    card.draw(ctx, card.x + cardW / 2, card.y + cardH / 2 - 8, iconSize);
    ctx.fillStyle = c.textMuted;
    ctx.font = "500 14px Inter, sans-serif";
    ctx.textAlign = "center";
    ctx.fillText(card.label, card.x + cardW / 2, card.y + cardH - 14);
    ctx.textAlign = "left";
  });

  // Vera card
  const veraCardX = veraX + (cardW - veraW) / 2;
  roundRect(ctx, veraCardX, veraY, veraW, veraH, 16);
  ctx.fillStyle = c.bgCard;
  ctx.fill();
  ...
  ctx.fillText("Vera", veraLogoCx - 18, veraY + 46);
  ctx.fillStyle = c.textMuted;
  ctx.font = "400 13px Inter, sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("AI Engineering Agent", veraCardX + veraW / 2, veraY + 72);
  ctx.textAlign = "left";

  // Footer
  ctx.fillStyle = `${c.border}60`;
  ctx.fillRect(72, H - 60, W - 144, 1);
  ctx.fillStyle = `${c.textMuted}80`;
...
async function main() {
  ...
  writeFileSync(
    ...
    await sharp(logo512).resize(180, 180).png().toBuffer(),
  );
  ...
  writeFileSync(
    ...
    await sharp(logo512).resize(192, 192).png().toBuffer(),
  );
